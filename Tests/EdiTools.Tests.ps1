...
Describe "Get-EdiFile" {
    ...
    $TestFiles = @(
        @{ "ContextName" = "Test file has ~ segment delimiter, no carriage return or line feed"; 
           "Path" = (Join-Path $TestDrive "$([System.Guid]::NewGuid()).txt");
           "FileContent" = $835Body; 
           "HasLineFeed" = $false; 
    ...
    foreach($testFile in $TestFiles) {
        Context "$($testFile['ContextName'])" {
            # create test file            
            Set-Content -Path $testFile["Path"] -Value $testFile["FileContent"]
            $ediFile = Get-EdiFile -InputObject $testFile["Path"]
