...
Describe "Get-EdiTransactionSet" {
    foreach($testFile in $TestFiles) {
        Context "$($testFile['ContextName'])" {
            ...
            Write-Host $path     
            Set-Content -Path $path -Value $testFile["FileContent"]
            Get-EdiFile -InputObject $path | Get-EdiTransactionSet -OutVariable ediTransactionSet

            It "is not null" {
                $ediTransactionSet | Should Not Be $null
            }

            It "has expected ST property values" {
                $ediTransactionSet.ST01 | Should Be "835"
                $ediTransactionSet.TransactionSetIdentifierCode | Should Be "835"

                $ediTransactionSet.ST02 | Should Be "07504123"
                $ediTransactionSet.TransactionSetControlNumber | Should Be "07504123"
            }
        }
    }
}